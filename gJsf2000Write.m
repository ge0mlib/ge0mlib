...
%Write [JsfHead,Head] to *.jsf file for Message Type 2000 (Sonar Virtual Ports Data; not annotated).
...
%function gJsf2000Write(JsfHead,Head,fNameNew,flTraceLenChanged), where
...
[fId, mes]=fopen(JsfHead.fName,'r+');if ~isempty(mes), error(['gJsfWrite2000: ' mes]);end;
if strcmp(fNameNew,JsfHead.fName), error('gJsfWrite2000: XtfHead.fName and fNameNew must be different.');end;
[fId2, mes]=fopen(fNameNew,'w');if ~isempty(mes), error(['gJsfWrite2000: ' mes]);end;
df=0;mm=0;nHead=find((JsfHead.HMessageType==2000)&(JsfHead.HChannelMulti==Head.HChannelMulti));
for m=1:size(JsfHead.HMessageType,2),
    if ~((JsfHead.HMessageType(m)==2000)&&(JsfHead.HChannelMulti(m)==Head.HChannelMulti)),
        ...
        %===End DataBlock Write
    elseif (JsfHead.HMessageType(m)==2000)&&(JsfHead.HChannelMulti(m)==Head.HChannelMulti),
        %===Begin JsfHeader Record Write
        fwrite(fId2,JsfHead.HMarkerForStart(m),'uint16'); %0-1// Marker for the Start of Header = 0x1601
        fwrite(fId2,JsfHead.HVersionOfProtocol(m),'uint8'); %2// Version of Protocol used
        fwrite(fId2,JsfHead.HSessionIdentifier(m),'uint8'); %3// Session Identifier
        ...
        %===End JsfHeader Record Write
        mm=mm+1;
        if nHead(mm)~=Head.HMessageNum(mm), error(['Error: gFJsfWrite2000, not correct Messages order for message num=' num2str(mm)]);end;        
        %===Begin Head Write for Message Type 2000 (Header length from JsfHead.HSizeFollowingMessage; Header data from Head)
        ...
        fwrite(fId2,Head.Source(mm),'uint8'); %8// Source, 1 = Sonar, 2 = Discover, 3 = ETSI 
        fwrite(fId2,Head.Reserved1(:,mm),'uint8'); %9-11// Reserved – Do not use
        fwrite(fId2,Head.String(1:JsfHead.HSizeFollowingMessage(m)-12,mm),'uint8'); %12-end// Virtual Ports Data
        %===End Head Write for Message Type 2000
        %===Begin checking for Number Bytes This Record
        HSizeFollowingMessageNew=ftell(fId2)-JsfHead.RSeek(m); %estimate
        if (HSizeFollowingMessageNew~=JsfHead.HSizeFollowingMessage(m))&&(~flTraceLenChanged),error(['Error: gFJsfWrite2000, not correct JsfHead.HSizeFollowingMessage for message num=' num2str(m)]);end;
