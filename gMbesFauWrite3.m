function gMbesFauWrite3(fNameNew,FauHead,Head)
...
switch fNameNew(end-2:end),
    case {'fau','FAU','fas','FAS'}, %.fas -- The 's' in the extension indicates that the location of the measurements is in stereographic coordinates.
        %GpsN.*100,'int32'; %1-4/S32/ The northing coordinate, in centimeters.
        %GpsE.*100,'int32'; %5-8/S32/ The easting coordinate, in centimeters.
        %Depth.*100,'int32'; %9-12/S32/ The depth value, in centimeters
        %UnixTime,'int32'; %13-16/S32/ The UNIX time, in seconds.
        %BeamAngle.*0.01,'int16'; %17-18/S16/ The beam angle, in 0.01 degrees. The angle is positive at starboard.
        %Heave.*0.02,'int8'; %19/S8/ The heave value, in 0.02 meters. The value is positive down.
        %Roll.*0.1,'int8'; %20/S8/ The roll value, in 0.1 degrees (0.2 for some equip). The angle is positive when the starboard side is down.
        %Quality,'uint8'; %21/U8/ A value representing the quality of the depth measurement. Each bit in the value has a specific meaning: Bit0to3-quality indicators. Bit4to6-reserved for flagging. Bit7-Valid is 0, rejected is 1. // Bit0to3 for RESON echosounders EIVA: 00b-Not detected (neither amplitude nor phase); xx01b-amplitude detection; xx10b-phase detection; xx11b-combined amplitude and phase detection. x1xxb-passed Brightness Test; 1xxxb-passed Collinearity Test. // Bit4to6 Vise 14.3 implementation of the flagging/rejection: 1001b-rejected by angle (e.g., >60 degrees); 0010b-only flagged by MapSpikes; 1010b-rejected based on MapSpikes.
        %Amplitude,'int8'; %22/S8/ The signal amplitude associated with the depth measurement. The unit of measure is unspecified.
        %Pitch.*0.1,'int8'; %23/S8/ The pitch value, in 0.1 degrees. The value is positive when the bow is up.
        %CentiSec,'uint8'; %24/U8/ The number of centiseconds to be added to the sec field.
        tmp=[flipud(reshape(typecast(int32(Head.GpsN.*100),'uint8'),4,numel(Head.GpsN)));flipud(reshape(typecast(int32(Head.GpsE.*100),'uint8'),4,numel(Head.GpsE)));flipud(reshape(typecast(int32(Head.Depth.*100),'uint8'),4,numel(Head.Depth)));flipud(reshape(typecast(int32(Head.UnixTime),'uint8'),4,numel(Head.UnixTime)));flipud(reshape(typecast(int16(Head.BeamAngle.*0.01),'uint8'),2,numel(Head.BeamAngle)));...
            typecast(int8(Head.Heave.*0.02),'uint8');typecast(int8(Head.Roll.*0.1),'uint8');typecast(uint8(Head.Quality),'uint8');typecast(int8(Head.Amplitude),'uint8');typecast(int8(Head.Pitch.*0.1),'uint8');typecast(uint8(Head.CentiSec),'uint8')];
    ...
    case {'fag','FAQ','fal','FAL'}, %.fag /.fal  -- The 'g' in the extension indicates that the location of the measurements is in geographic coordinates. Similarly, the 'l' was for 'Latitude/Longitude'.
        %GpsLat.*1e6,'int32'; %1-4/S32/ The latitude (dimention not described).
        %GpsLon.*1e6,'int32'; %5-8/S32/ The longitude (dimention not described).
        %Depth.*100,'int32'; %9-12/S32/ The depth value, in centimeters
        %UnixTime,'int32'; %13-16/S32/ The UNIX time, in seconds.
        %BeamAngle.*0.01,'int16'; %17-18/S16/ The beam angle, in 0.01 degrees. The angle is positive at starboard.
        %Heave.*0.02,'int8'; %19/S8/ The heave value, in 0.02 meters. The value is positive down.
        ...
        %Pitch.*0.1,'int8'; %23/S8/ The pitch value, in 0.1 degrees. The value is positive when the bow is up.
        %CentiSec,'uint8'; %24/U8/ The number of centiseconds to be added to the sec field.
        tmp=[flipud(reshape(typecast(int32(Head.GpsLat.*1e6),'uint8'),4,numel(Head.GpsN)));flipud(reshape(typecast(int32(Head.GpsLon.*1e6),'uint8'),4,numel(Head.GpsE)));flipud(reshape(typecast(int32(Head.Depth.*100),'uint8'),4,numel(Head.Depth)));flipud(reshape(typecast(int32(Head.UnixTime),'uint8'),4,numel(Head.UnixTime)));flipud(reshape(typecast(int16(Head.BeamAngle.*0.01),'uint8'),2,numel(Head.BeamAngle)));...
            typecast(int8(Head.Heave.*0.02),'uint8');typecast(int8(Head.Roll.*0.1),'uint8');typecast(uint8(Head.Quality),'uint8');typecast(int8(Head.Amplitude),'uint8');typecast(int8(Head.Pitch.*0.1),'uint8');typecast(uint8(Head.CentiSec),'uint8')];
        fwrite(fId,tmp,'uint8');
    case {'fu2','FU2'}, %.fu2 -- (1) The implicit roll multiplier is 0.2 (rather than 0.1). (2) The 2-byte Beam Angle field is substituted by: 1-byte Beam Number (value stored as a char; thus its range is between -128 and 127); 1-byte Mean Error of the Depth Relative to its Neighborhood (value stored as an unsigned char; thus its range is  between 0 and 255).
        ...
        %Depth.*100,'int32'; %9-12/S32/ The depth value, in centimeters
        %UnixTime,'int32'; %13-16/S32/ The UNIX time, in seconds.
        %BeamNumber,'int8'; %17/S8 Beam Number (value stored as a char; thus its range is between -128 and 127);
        %MeanError.*100,'uint8'; %18/U8 Mean Error of the Depth Relative to its doo Neighborhood (value stored as an unsigned char; thus its range is  between 0 and 255).
        %Heave.*0.02,'int8'; %19/S8/ The heave value, in 0.02 meters. The value is positive down.
        %Roll.*0.1,'int8'; %20/S8/ The roll value, in 0.1 degrees (0.2 for some equip). The angle is positive when the starboard side is down.
        %Quality,'uint8'; %21/U8/ A value representing the quality of the depth measurement. Each bit in the value has a specific meaning: Bit0to3-quality indicators. Bit4to6-reserved for flagging. Bit7-Valid is 0, rejected is 1. // Bit0to3 for RESON echosounders EIVA: 00b-Not detected (neither amplitude nor phase); xx01b-amplitude detection; xx10b-phase detection; xx11b-combined amplitude and phase detection. x1xxb-passed Brightness Test; 1xxxb-passed Collinearity Test. // Bit4to6 Vise 14.3 implementation of the flagging/rejection: 1001b-rejected by angle (e.g., >60 degrees); 0010b-only flagged by MapSpikes; 1010b-rejected based on MapSpikes.
        %Amplitude,'int8'; %22/S8/ The signal amplitude associated with the depth measurement. The unit of measure is unspecified.
        %Pitch.*0.1,'int8'; %23/S8/ The pitch value, in 0.1 degrees. The value is positive when the bow is up.
        %CentiSec,'uint8'; %24/U8/ The number of centiseconds to be added to the sec field.
        tmp=[flipud(reshape(typecast(int32(Head.GpsN.*100),'uint8'),4,numel(Head.GpsN)));flipud(reshape(typecast(int32(Head.GpsE.*100),'uint8'),4,numel(Head.GpsE)));flipud(reshape(typecast(int32(Head.Depth.*100),'uint8'),4,numel(Head.Depth)));flipud(reshape(typecast(int32(Head.UnixTime),'uint8'),4,numel(Head.UnixTime)));typecast(int8(Head.BeamNumber),'uint8')';typecast(uint8(Head.MeanError.*100),'uint8')';...
            typecast(int8(Head.Heave.*0.02),'uint8');typecast(int8(Head.Roll.*0.1),'uint8');typecast(uint8(Head.Quality),'uint8');typecast(int8(Head.Amplitude),'uint8');typecast(int8(Head.Pitch.*0.1),'uint8');typecast(uint8(Head.CentiSec),'uint8')];
        fwrite(fId,tmp,'uint8');
end;
fclose(fId);
