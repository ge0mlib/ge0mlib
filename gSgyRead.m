...
SgyHead.Line=fread(fId,1,'int32'); %Line number.  For 3-D poststack data, this will typically contain the in-line number.
SgyHead.Reel=fread(fId,1,'int32'); %Reel number.
SgyHead.DataTracePerEnsemble=fread(fId,1,'int16'); %Number of data traces per ensemble.  Mandatory for prestack data.
SgyHead.AuxiliaryTracePerEnsemble=fread(fId,1,'int16'); %Number of auxiliary traces per ensemble.  Mandatory for prestack data.
SgyHead.dt=fread(fId,1,'int16'); %Sample interval in microseconds (µs).  Mandatory for all data types.
SgyHead.dtOrig=fread(fId,1,'int16'); %Sample interval in microseconds  (µs) of original field recording.
SgyHead.ns=fread(fId,1,'int16'); %Number of samples per data trace.  Mandatory for all types of data. Note: The sample interval and number of samples in the Binary File Header should be for the primary set of seismic data traces in the file.
SgyHead.nsOrig=fread(fId,1,'int16'); %Number of samples per data trace for original field recording.
SgyHead.DataSampleFormat=fread(fId,1,'int16'); %Data sample format code.  Mandatory for all data. 1=4-byte IBM floating-point; 2=4-byte, two's complement integer; 3=2-byte, two's complement integer; 4=4-byte fixed-point with gain (obsolete); 5=4-byte IEEE floating-point; 6=Not currently used; 7=Not currently used; 8=1-byte, two's complement integer.
SgyHead.EnsembleFold=fread(fId,1,'int16'); %Ensemble fold - The expected number of data traces per trace ensemble (e.g. the CMP fold). Highly recommended for all types of data.
...
SgyHead.MeasurementSystem=fread(fId,1,'int16'); %Measurement system: Highly recommended for all types of data.  If Location Data stanzas are included in the file, this entry must agree with the Location Data stanza.  If there is a disagreement, the last Location Data stanza is the controlling authority. 1=Meters; 2=Feet.
...
SgyHead.Unassigned1=fread(fId,120,'int16'); %Unassigned1
SgyHead.SegyFormatRevisionNumber=fread(fId,1,'int16'); %SEG Y Format Revision Number. This is a 16-bit unsigned value with a Q-point between the first and second bytes. Thus for SEG Y Revision 1.0, as defined in this document, this will be recorded as 0100. This field is mandatory for all versions of SEG Y, although a value of zero indicates "traditional" SEG Y conforming to the 1975 standard.
SgyHead.FixedLengthTraceFlag=fread(fId,1,'int16'); %Fixed length trace flag. A value of one indicates that all traces in this SEG Y file are guaranteed to have the same sample interval and number of samples, as specified in Textual File Header bytes 3217-3218 and 3221-3222. A value of zero indicates that the length of the traces in the file may vary and the number of samples in bytes 115-116 of the Trace Header must be examined to determine the actual length of each trace. This field is mandatory for all versions of SEG Y, although a value of zero indicates "traditional" SEG Y conforming to the 1975 standard.
...
SgyHead.Unassigned2=fread(fId,47,'int16'); %Unassigned2
...
for n=1:NTrace,
    ...
    Head.TraceNumber(n)=fread(fId,1,'int32'); %Trace number within the original field record. Highly recommended for all types of data.
    ...
    Head.SourceSurfaceElevation(n)=fread(fId,1,'int32'); %////Surface elevation at source.
    Head.SourceDepth(n)=fread(fId,1,'int32'); %////Source depth below surface (a positive number).
    ...
    Head.SourceWaterDepth(n)=fread(fId,1,'int32'); %////Water depth at source.
    ...
    Head.SourceGroupScalar(n)=fread(fId,1,'int16'); %Scalar to be applied to all coordinates specified in Trace Header bytes 73-88 and to bytes Trace Header 181-188 to give the real value.  Scalar = 1, +10, +100, +1000, or +10,000.  If positive, scalar is used as a multiplier; if negative, scalar is used as divisor.
    ...
    Head.SourceX(n)=fread(fId,1,'int32'); %//Source coordinate - X.
    Head.SourceY(n)=fread(fId,1,'int32'); %//Source coordinate - Y.
    Head.GroupX(n)=fread(fId,1,'int32'); %//Group coordinate - X.
    Head.GroupY(n)=fread(fId,1,'int32'); %//Group coordinate - Y.
    Head.CoordinateUnits(n)=fread(fId,1,'int16'); %Coordinate units: 1=Length (meters or feet); 2=Seconds of arc; 3=Decimal degrees; 4=Degrees, minutes, seconds (DMS). Note: To encode +-DDDMMSS bytes 89-90 equal= +-DDD*10^4 + MM*10^2 + SS with bytes 71-72 set to 1; To encode +-DDDMMSS.ss bytes 89-90 equal= +-DDD*10^6 + MM*10^4 + SS*10^2 with bytes 71-72 set to -100.
    ...
    Head.SweepFrequenceStart(n)=fread(fId,1,'int16'); %Sweep frequency at start (Hz).
    Head.SweepFrequenceEnd(n)=fread(fId,1,'int16'); %Sweep frequency at end (Hz).
    Head.SweepLength(n)=fread(fId,1,'int16'); %Sweep length in milliseconds.
    ...
    Head.TimeBaseCode(n)=fread(fId,1,'int16'); %Time basis code: 1=Local; 2=GMT (Greenwich Mean Time); 3=Other, should be explained in a user defined stanza in the Extended Textual File Header; 4=UTC (Coordinated Universal Time).
    ...
    Head.GeophoneGroupNumberRoll(n)=fread(fId,1,'int16'); %Geophone group number of roll switch position one.
    Head.GeophoneGroupNumberFirstTraceOrigField(n)=fread(fId,1,'int16'); %Geophone group number of trace number one within original field record.
    Head.GeophoneGroupNumberLastTraceOrigField(n)=fread(fId,1,'int16'); %Geophone group number of last trace within original field record.
    Head.GapSize(n)=fread(fId,1,'int16'); %Gap size (total number of groups dropped).
    Head.OverTravel(n)=fread(fId,1,'int16'); %Over travel associated with taper at beginning or end of line: 1=down (or behind); 2=up (or ahead).
    ...
    Head.UnassignedInt1(n)=fread(fId,1,'int32'); %Unassigned — For optional information.
    Head.UnassignedInt2(n)=fread(fId,1,'int32'); %Unassigned — For optional information.
