function [FauHead,Head]=gMbesFauRead2(fName,Endian,BeamNum)
%Read FAU variables [FauHead,Head] from file.
%function function [FauHead,Head]=gMbesFauRead2(fName,Endian), where
...
%Example: [FauHead,Head]=gMbesFauRead2('c:\temp\1.fau','b',[]);scatter3(Head.GpsE(1:10000),Head.GpsN(1:10000),Head.Depth(1:10000),[],Head.Depth(1:10000),'.');
...
switch FauHead.fName(end-2:end),
    case {'fau','FAU','fas','FAS'}, %.fas -- The 's' in the extension indicates that the location of the measurements is in stereographic coordinates.
        ...
        Head.MessageNum=1:NDtg;
        fseek(fId,SeekDtgStart+0,'bof');Head.GpsN=fread(fId,NDtg,'int32',20)'./100; %1-4/S32/ The northing coordinate, in centimeters.
        fseek(fId,SeekDtgStart+4,'bof');Head.GpsE=fread(fId,NDtg,'int32',20)'./100; %5-8/S32/ The easting coordinate, in centimeters.
        fseek(fId,SeekDtgStart+8,'bof');Head.Depth=fread(fId,NDtg,'int32',20)'./100; %9-12/S32/ The depth value, in centimeters
        fseek(fId,SeekDtgStart+12,'bof');Head.UnixTime=fread(fId,NDtg,'int32',20)'; %13-16/S32/ The UNIX time, in seconds.
        fseek(fId,SeekDtgStart+16,'bof');Head.BeamAngle=fread(fId,NDtg,'int16',22)'./0.01; %17-18/S16/ The beam angle, in 0.01 degrees. The angle is positive at starboard.
        fseek(fId,SeekDtgStart+18,'bof');Head.Heave=fread(fId,NDtg,'int8',23)'./0.02; %19/S8/ The heave value, in 0.02 meters. The value is positive down.
        fseek(fId,SeekDtgStart+19,'bof');Head.Roll=fread(fId,NDtg,'int8',23)'./0.1; %20/S8/ The roll value, in 0.1 degrees (0.2 for some equip). The angle is positive when the starboard side is down.
        fseek(fId,SeekDtgStart+20,'bof');Head.Quality=fread(fId,NDtg,'uint8',23)'; %21/U8/ A value representing the quality of the depth measurement. Each bit in the value has a specific meaning: Bit0to3-quality indicators. Bit4to6-reserved for flagging. Bit7-Valid is 0, rejected is 1. // Bit0to3 for RESON echosounders EIVA: 00b-Not detected (neither amplitude nor phase); xx01b-amplitude detection; xx10b-phase detection; xx11b-combined amplitude and phase detection. x1xxb-passed Brightness Test; 1xxxb-passed Collinearity Test. // Bit4to6 Vise 14.3 implementation of the flagging/rejection: 1001b-rejected by angle (e.g., >60 degrees); 0010b-only flagged by MapSpikes; 1010b-rejected based on MapSpikes.
        fseek(fId,SeekDtgStart+21,'bof');Head.Amplitude=fread(fId,NDtg,'int8',23)'; %22/S8/ The signal amplitude associated with the depth measurement. The unit of measure is unspecified.
        fseek(fId,SeekDtgStart+22,'bof');Head.Pitch=fread(fId,NDtg,'int8',23)'./0.1; %23/S8/ The pitch value, in 0.1 degrees. The value is positive when the bow is up.
        fseek(fId,SeekDtgStart+23,'bof');Head.CentiSec=fread(fId,NDtg,'uint8',23)'; %24/U8/ The number of centiseconds to be added to the sec field.
    case {'fag','FAQ','fal','FAL'}, %.fag /.fal  -- The 'g' in the extension indicates that the location of the measurements is in geographic coordinates. Similarly, the 'l' was for 'Latitude/Longitude'.
        ...
        Head.MessageNum=1:NDtg;
        fseek(fId,SeekDtgStart+0,'bof');Head.GpsLat=fread(fId,NDtg,'int32',20)'./1e6; %1-4/S32/ The latitude (dimention not described).
        fseek(fId,SeekDtgStart+4,'bof');Head.GpsLon=fread(fId,NDtg,'int32',20)'./1e6; %5-8/S32/ The longitude (dimention not described).
        fseek(fId,SeekDtgStart+8,'bof');Head.Depth=fread(fId,NDtg,'int32',20)'./100; %9-12/S32/ The depth value, in centimeters
        fseek(fId,SeekDtgStart+12,'bof');Head.UnixTime=fread(fId,NDtg,'int32',20)'; %13-16/S32/ The UNIX time, in seconds.
        fseek(fId,SeekDtgStart+16,'bof');Head.BeamAngle=fread(fId,NDtg,'int16',22)'./0.01; %17-18/S16/ The beam angle, in 0.01 degrees. The angle is positive at starboard.
        fseek(fId,SeekDtgStart+18,'bof');Head.Heave=fread(fId,NDtg,'int8',23)'./0.02; %19/S8/ The heave value, in 0.02 meters. The value is positive down.
    ...
    case {'fu2','FU2'}, %.fu2 -- (1) The implicit roll multiplier is 0.2 (rather than 0.1). (2) The 2-byte Beam Angle field is substituted by: 1-byte Beam Number (value stored as a char; thus its range is between -128 and 127); 1-byte Mean Error of the Depth Relative to its Neighborhood (value stored as an unsigned char; thus its range is  between 0 and 255).
        ...
        Head.MessageNum=1:NDtg;
        fseek(fId,SeekDtgStart+0,'bof');Head.GpsLat=fread(fId,NDtg,'int32',20)'./100; %1-4/S32/ The latitude (dimention not described).
        fseek(fId,SeekDtgStart+4,'bof');Head.GpsLon=fread(fId,NDtg,'int32',20)'./100; %5-8/S32/ The longitude (dimention not described).
        fseek(fId,SeekDtgStart+8,'bof');Head.Depth=fread(fId,NDtg,'int32',20)'./100; %9-12/S32/ The depth value, in centimeters
        fseek(fId,SeekDtgStart+12,'bof');Head.UnixTime=fread(fId,NDtg,'int32',20)'; %13-16/S32/ The UNIX time, in seconds.
        fseek(fId,SeekDtgStart+16,'bof');Head.BeamNumber=fread(fId,NDtg,'int8',23)'; %17/S8 Beam Number (value stored as a char; thus its range is between -128 and 127);
        fseek(fId,SeekDtgStart+17,'bof');Head.MeanError=fread(fId,NDtg,'uint8',23)'./100; %18/U8 Mean Error of the Depth Relative to its doo Neighborhood (value stored as an unsigned char; thus its range is  between 0 and 255).
        fseek(fId,SeekDtgStart+18,'bof');Head.Heave=fread(fId,NDtg,'int8',23)'./0.02; %19/S8/ The heave value, in 0.02 meters. The value is positive down.
        fseek(fId,SeekDtgStart+19,'bof');Head.Roll=fread(fId,NDtg,'int8',23)'./0.2; %20/S8/ The roll value, in 0.1 degrees (0.2 for some equip). The angle is positive when the starboard side is down.
        fseek(fId,SeekDtgStart+20,'bof');Head.Quality=fread(fId,NDtg,'uint8',23)'; %21/U8/ A value representing the quality of the depth measurement. Each bit in the value has a specific meaning: Bit0to3-quality indicators. Bit4to6-reserved for flagging. Bit7-Valid is 0, rejected is 1. // Bit0to3 for RESON echosounders EIVA: 00b-Not detected (neither amplitude nor phase); xx01b-amplitude detection; xx10b-phase detection; xx11b-combined amplitude and phase detection. x1xxb-passed Brightness Test; 1xxxb-passed Collinearity Test. // Bit4to6 Vise 14.3 implementation of the flagging/rejection: 1001b-rejected by angle (e.g., >60 degrees); 0010b-only flagged by MapSpikes; 1010b-rejected based on MapSpikes.
        fseek(fId,SeekDtgStart+21,'bof');Head.Amplitude=fread(fId,NDtg,'int8',23)'; %22/S8/ The signal amplitude associated with the depth measurement. The unit of measure is unspecified.
        fseek(fId,SeekDtgStart+22,'bof');Head.Pitch=fread(fId,NDtg,'int8',23)'./0.1; %23/S8/ The pitch value, in 0.1 degrees. The value is positive when the bow is up.
        fseek(fId,SeekDtgStart+23,'bof');Head.CentiSec=fread(fId,NDtg,'uint8',23)'; %24/U8/ The number of centiseconds to be added to the sec field.
