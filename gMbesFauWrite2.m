function gMbesFauWrite2(fNameNew,FauHead,Head)
%Write FAU variables [FauHead,Head] to file.
%function function gMbesFauWrite2(fName,FauHead,Head), where
...
%Example: gMbesFauWrite2('c:\temp\2.fau',FauHead,Head);
...
switch fNameNew(end-2:end),
    case {'fau','FAU','fas','FAS'}, %.fas -- The 's' in the extension indicates that the location of the measurements is in stereographic coordinates.
        fseek(fId,SeekDtgStart,'bof');fwrite(fId,Head.CentiSec,'uint8',23); %24/U8/ The number of centiseconds to be added to the sec field.
        fseek(fId,SeekDtgStart+0,'bof');fwrite(fId,Head.GpsN(1).*100,'int32');fwrite(fId,Head.GpsN(2:end).*100,'int32',20); %1-4/S32/ The northing coordinate, in centimeters.
        fseek(fId,SeekDtgStart+4,'bof');fwrite(fId,Head.GpsE(1).*100,'int32');fwrite(fId,Head.GpsE(2:end).*100,'int32',20); %5-8/S32/ The easting coordinate, in centimeters.
        fseek(fId,SeekDtgStart+8,'bof');fwrite(fId,Head.Depth(1).*100,'int32');fwrite(fId,Head.Depth(2:end).*100,'int32',20); %9-12/S32/ The depth value, in centimeters
        fseek(fId,SeekDtgStart+12,'bof');fwrite(fId,Head.UnixTime(1),'int32');fwrite(fId,Head.UnixTime(2:end),'int32',20); %13-16/S32/ The UNIX time, in seconds.
        fseek(fId,SeekDtgStart+16,'bof');fwrite(fId,Head.BeamAngle(1).*0.01,'int16');fwrite(fId,Head.BeamAngle(2:end).*0.01,'int16',22); %17-18/S16/ The beam angle, in 0.01 degrees. The angle is positive at starboard.
        fseek(fId,SeekDtgStart+18,'bof');fwrite(fId,Head.Heave(1).*0.02,'int8');fwrite(fId,Head.Heave(2:end).*0.02,'int8',23); %19/S8/ The heave value, in 0.02 meters. The value is positive down.
        fseek(fId,SeekDtgStart+19,'bof');fwrite(fId,Head.Roll(1).*0.1,'int8');fwrite(fId,Head.Roll(2:end).*0.1,'int8',23); %20/S8/ The roll value, in 0.1 degrees (0.2 for some equip). The angle is positive when the starboard side is down.
        fseek(fId,SeekDtgStart+20,'bof');fwrite(fId,Head.Quality(1),'uint8');fwrite(fId,Head.Quality(2:end),'uint8',23); %21/U8/ A value representing the quality of the depth measurement. Each bit in the value has a specific meaning: Bit0to3-quality indicators. Bit4to6-reserved for flagging. Bit7-Valid is 0, rejected is 1. // Bit0to3 for RESON echosounders EIVA: 00b-Not detected (neither amplitude nor phase); xx01b-amplitude detection; xx10b-phase detection; xx11b-combined amplitude and phase detection. x1xxb-passed Brightness Test; 1xxxb-passed Collinearity Test. // Bit4to6 Vise 14.3 implementation of the flagging/rejection: 1001b-rejected by angle (e.g., >60 degrees); 0010b-only flagged by MapSpikes; 1010b-rejected based on MapSpikes.
        fseek(fId,SeekDtgStart+21,'bof');fwrite(fId,Head.Amplitude(1),'int8');fwrite(fId,Head.Amplitude(2:end),'int8',23); %22/S8/ The signal amplitude associated with the depth measurement. The unit of measure is unspecified.
        fseek(fId,SeekDtgStart+22,'bof');fwrite(fId,Head.Pitch(1).*0.1,'int8');fwrite(fId,Head.Pitch(2:end).*0.1,'int8',23); %23/S8/ The pitch value, in 0.1 degrees. The value is positive when the bow is up.
    case {'fag','FAQ','fal','FAL'}, %.fag /.fal  -- The 'g' in the extension indicates that the location of the measurements is in geographic coordinates. Similarly, the 'l' was for 'Latitude/Longitude'.
        fseek(fId,SeekDtgStart,'bof');fwrite(fId,Head.CentiSec,'uint8',23); %24/U8/ The number of centiseconds to be added to the sec field.
        fseek(fId,SeekDtgStart+0,'bof');fwrite(fId,Head.GpsLat(1).*1e6,'int32');fwrite(fId,Head.GpsLat(2:end).*100,'int32',20); %1-4/S32/ The latitude (dimention not described).
        fseek(fId,SeekDtgStart+4,'bof');fwrite(fId,Head.GpsLon(1).*1e6,'int32');fwrite(fId,Head.GpsLon(2:end).*100,'int32',20); %5-8/S32/ The longitude (dimention not described).
        fseek(fId,SeekDtgStart+8,'bof');fwrite(fId,Head.Depth(1).*100,'int32');fwrite(fId,Head.Depth(2:end).*100,'int32',20); %9-12/S32/ The depth value, in centimeters
        fseek(fId,SeekDtgStart+12,'bof');fwrite(fId,Head.UnixTime(1),'int32');fwrite(fId,Head.UnixTime(2:end),'int32',20); %13-16/S32/ The UNIX time, in seconds.
        fseek(fId,SeekDtgStart+16,'bof');fwrite(fId,Head.BeamAngle(1).*0.01,'int16');fwrite(fId,Head.BeamAngle(2:end).*0.01,'int16',22); %17-18/S16/ The beam angle, in 0.01 degrees. The angle is positive at starboard.
        fseek(fId,SeekDtgStart+18,'bof');fwrite(fId,Head.Heave(1).*0.02,'int8');fwrite(fId,Head.Heave(2:end).*0.02,'int8',23); %19/S8/ The heave value, in 0.02 meters. The value is positive down.
    ...
    case {'fu2','FU2'}, %.fu2 -- (1) The implicit roll multiplier is 0.2 (rather than 0.1). (2) The 2-byte Beam Angle field is substituted by: 1-byte Beam Number (value stored as a char; thus its range is between -128 and 127); 1-byte Mean Error of the Depth Relative to its Neighborhood (value stored as an unsigned char; thus its range is  between 0 and 255).
        ...
        fseek(fId,SeekDtgStart+8,'bof');fwrite(fId,Head.Depth(1).*100,'int32');fwrite(fId,Head.Depth(2:end).*100,'int32',20); %9-12/S32/ The depth value, in centimeters
        fseek(fId,SeekDtgStart+12,'bof');fwrite(fId,Head.UnixTime(1),'int32');fwrite(fId,Head.UnixTime(2:end),'int32',20); %13-16/S32/ The UNIX time, in seconds.
        fseek(fId,SeekDtgStart+16,'bof');fwrite(fId,Head.BeamNumber(1),'int8');fwrite(fId,Head.BeamNumber(2:end),'int16',23); %17/S8 Beam Number (value stored as a char; thus its range is between -128 and 127);
        fseek(fId,SeekDtgStart+17,'bof');fwrite(fId,Head.MeanError(1).*100,'uint8');fwrite(fId,Head.MeanError(2:end).*100,'int16',23); %18/U8 Mean Error of the Depth Relative to its doo Neighborhood (value stored as an unsigned char; thus its range is  between 0 and 255).
        fseek(fId,SeekDtgStart+18,'bof');fwrite(fId,Head.Heave(1).*0.02,'int8');fwrite(fId,Head.Heave(2:end).*0.02,'int8',23); %19/S8/ The heave value, in 0.02 meters. The value is positive down.
        fseek(fId,SeekDtgStart+19,'bof');fwrite(fId,Head.Roll(1).*0.2,'int8');fwrite(fId,Head.Roll(2:end).*0.2,'int8',23); %20/S8/ The roll value, in 0.1 degrees (0.2 for some equip). The angle is positive when the starboard side is down.
        fseek(fId,SeekDtgStart+20,'bof');fwrite(fId,Head.Quality(1),'uint8');fwrite(fId,Head.Quality(2:end),'uint8',23); %21/U8/ A value representing the quality of the depth measurement. Each bit in the value has a specific meaning: Bit0to3-quality indicators. Bit4to6-reserved for flagging. Bit7-Valid is 0, rejected is 1. // Bit0to3 for RESON echosounders EIVA: 00b-Not detected (neither amplitude nor phase); xx01b-amplitude detection; xx10b-phase detection; xx11b-combined amplitude and phase detection. x1xxb-passed Brightness Test; 1xxxb-passed Collinearity Test. // Bit4to6 Vise 14.3 implementation of the flagging/rejection: 1001b-rejected by angle (e.g., >60 degrees); 0010b-only flagged by MapSpikes; 1010b-rejected based on MapSpikes.
        fseek(fId,SeekDtgStart+21,'bof');fwrite(fId,Head.Amplitude(1),'int8');fwrite(fId,Head.Amplitude(2:end),'int8',23); %22/S8/ The signal amplitude associated with the depth measurement. The unit of measure is unspecified.
        fseek(fId,SeekDtgStart+22,'bof');fwrite(fId,Head.Pitch(1).*0.1,'int8');fwrite(fId,Head.Pitch(2:end).*0.1,'int8',23); %23/S8/ The pitch value, in 0.1 degrees. The value is positive when the bow is up.
end;
