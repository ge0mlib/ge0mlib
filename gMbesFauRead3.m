function [FauHead,Head]=gMbesFauRead3(fName,Endian,BeamNum)
%Read FAU variables [FauHead,Head] from file.
%function function [FauHead,Head]=gMbesFauRead3(fName,Endian,BeamNum), where
...
%Example: [FauHead,Head]=gMbesFauRead3('c:\temp\1.fau','b',[]);scatter3(Head.GpsE(1:10000),Head.GpsN(1:10000),Head.Depth(1:10000),[],Head.Depth(1:10000),'.');
...
%===Begin NDtg Read
SeekDtgStart=ftell(fId);NDtg=(fSize-SeekDtgStart)./24;tmp=reshape(fread(fId,NDtg.*24,'uint8=>uint8'),24,NDtg);
switch FauHead.fName(end-2:end),
    case {'fau','FAU','fas','FAS'}, %.fas -- The 's' in the extension indicates that the location of the measurements is in stereographic coordinates.
        %GpsN='int32'./100; %1-4/S32/ The northing coordinate, in centimeters.
        %GpsE='int32'./100; %5-8/S32/ The easting coordinate, in centimeters.
        %Depth='int32'./100; %9-12/S32/ The depth value, in centimeters
        %UnixTime='int32'; %13-16/S32/ The UNIX time, in seconds.
        %BeamAngle='int16'./0.01; %17-18/S16/ The beam angle, in 0.01 degrees. The angle is positive at starboard.
        %Heave='int8'./0.02; %19/S8/ The heave value, in 0.02 meters. The value is positive down.
        %Roll='int8'./0.1; %20/S8/ The roll value, in 0.1 degrees (0.2 for some equip). The angle is positive when the starboard side is down.
        ...
        %Pitch='int8'./0.1; %23/S8/ The pitch value, in 0.1 degrees. The value is positive when the bow is up.
        %CentiSec='uint8'; %24/U8/ The number of centiseconds to be added to the sec field.
        Head=struct('MessageNum',1:NDtg,'UnixTime',double(typecast(reshape(flipud(tmp(13:16,:)),1,NDtg.*4),'int32')),'CentiSec',double(typecast(tmp(24,:),'uint8')),'GpsN',double(typecast(reshape(flipud(tmp(1:4,:)),1,NDtg.*4),'int32'))./100,'GpsE',double(typecast(reshape(flipud(tmp(5:8,:)),1,NDtg.*4),'int32'))./100,...
            'BeamAngle',double(typecast(reshape(flipud(tmp(17:18,:)),1,NDtg.*2),'int16'))./0.01,'Heave',double(typecast(tmp(19,:),'int8'))./0.02,'Roll',double(typecast(tmp(20,:),'int8'))./0.1,'Pitch',double(typecast(tmp(23,:),'int8'))./0.1,'Depth',double(typecast(reshape(flipud(tmp(9:12,:)),1,NDtg.*4),'int32'))./100,'Amplitude',double(typecast(tmp(22,:),'int8')),'Quality',double(typecast(tmp(21,:),'uint8')));
    case {'fag','FAQ','fal','FAL'}, %.fag /.fal  -- The 'g' in the extension indicates that the location of the measurements is in geographic coordinates. Similarly, the 'l' was for 'Latitude/Longitude'.
        %GpsLat='int32'./1e6; %1-4/S32/ The latitude (dimention not described).
        %GpsLon='int32'./1e6; %5-8/S32/ The longitude (dimention not described).
        %Depth='int32'./100; %9-12/S32/ The depth value, in centimeters
        %UnixTime='int32'; %13-16/S32/ The UNIX time, in seconds.
        %BeamAngle='int16'./0.01; %17-18/S16/ The beam angle, in 0.01 degrees. The angle is positive at starboard.
        %Heave='int8'./0.02; %19/S8/ The heave value, in 0.02 meters. The value is positive down.
        ...
        %Pitch='int8'./0.1; %23/S8/ The pitch value, in 0.1 degrees. The value is positive when the bow is up.
        %CentiSec='uint8'; %24/U8/ The number of centiseconds to be added to the sec field.
        Head=struct('MessageNum',1:NDtg,'UnixTime',double(typecast(reshape(flipud(tmp(13:16,:)),1,NDtg.*4),'int32')),'CentiSec',double(typecast(tmp(24,:),'uint8')),'GpsLat',double(typecast(reshape(flipud(tmp(1:4,:)),1,NDtg.*4),'int32'))./1e6,'GpsLon',double(typecast(reshape(flipud(tmp(5:8,:)),1,NDtg.*4),'int32'))./1e6,...
            'BeamAngle',double(typecast(reshape(flipud(tmp(17:18,:)),1,NDtg.*2),'int16'))./0.01,'Heave',double(typecast(tmp(19,:),'int8'))./0.02,'Roll',double(typecast(tmp(20,:),'int8'))./0.1,'Pitch',double(typecast(tmp(23,:),'int8'))./0.1,'Depth',double(typecast(reshape(flipud(tmp(9:12,:)),1,NDtg.*4),'int32'))./100,'Amplitude',double(typecast(tmp(22,:),'int8')),'Quality',double(typecast(tmp(21,:),'uint8')));
    case {'fu2','FU2'}, %.fu2 -- (1) The implicit roll multiplier is 0.2 (rather than 0.1). (2) The 2-byte Beam Angle field is substituted by: 1-byte Beam Number (value stored as a char; thus its range is between -128 and 127); 1-byte Mean Error of the Depth Relative to its Neighborhood (value stored as an unsigned char; thus its range is  between 0 and 255).
        ...
        %MeanError='uint8'./100; %18/U8 Mean Error of the Depth Relative to its doo Neighborhood (value stored as an unsigned char; thus its range is  between 0 and 255).
        %Heave='int8'./0.02; %19/S8/ The heave value, in 0.02 meters. The value is positive down.
        %Roll='int8'./0.1; %20/S8/ The roll value, in 0.1 degrees (0.2 for some equip). The angle is positive when the starboard side is down.
        %Quality='uint8'; %21/U8/ A value representing the quality of the depth measurement. Each bit in the value has a specific meaning: Bit0to3-quality indicators. Bit4to6-reserved for flagging. Bit7-Valid is 0, rejected is 1. // Bit0to3 for RESON echosounders EIVA: 00b-Not detected (neither amplitude nor phase); xx01b-amplitude detection; xx10b-phase detection; xx11b-combined amplitude and phase detection. x1xxb-passed Brightness Test; 1xxxb-passed Collinearity Test. // Bit4to6 Vise 14.3 implementation of the flagging/rejection: 1001b-rejected by angle (e.g., >60 degrees); 0010b-only flagged by MapSpikes; 1010b-rejected based on MapSpikes.
        %Amplitude='int8'; %22/S8/ The signal amplitude associated with the depth measurement. The unit of measure is unspecified.
        ...
        %CentiSec='uint8'; %24/U8/ The number of centiseconds to be added to the sec field.
        Head=struct('MessageNum',1:NDtg,'UnixTime',double(typecast(reshape(flipud(tmp(13:16,:)),1,NDtg.*4),'int32')),'CentiSec',double(typecast(tmp(24,:),'uint8')),'GpsN',double(typecast(reshape(flipud(tmp(1:4,:)),1,NDtg.*4),'int32'))./100,'GpsE',double(typecast(reshape(flipud(tmp(5:8,:)),1,NDtg.*4),'int32'))./100,...
            'BeamNumber',double(typecast(tmp(17,:),'int8')),'Heave',double(typecast(tmp(19,:),'int8'))./0.02,'Roll',double(typecast(tmp(20,:),'int8'))./0.1,'Pitch',double(typecast(tmp(23,:),'int8'))./0.1,'Depth',double(typecast(reshape(flipud(tmp(9:12,:)),1,NDtg.*4),'int32'))./100,'MeanError',double(typecast(tmp(18,:),'uint8'))./100,'Amplitude',double(typecast(tmp(22,:),'int8')),'Quality',double(typecast(tmp(21,:),'uint8')));
end;
fclose(fId);
%===End NDtg Read
