...
%Example: gSgyWrite(SgyHead,Head,Data,'c:\temp\1new.sgy');
...
for n=1:LenHead,
    ...
    fwrite(fId,Head.TraceSequenceLine(n),'int32'); %1-4 Trace sequence number within line — Numbers continue to increase if the same line continues across multiple SEG Y files. Highly recommended for all types of data.
    fwrite(fId,Head.TraceSequenceFile(n),'int32'); %5-7 Trace sequence number within SEG Y file — Each file starts with trace sequence one.
    fwrite(fId,Head.FieldRecord(n),'int32'); %9-12 Original field record number. Highly recommended for all types of data.
    fwrite(fId,Head.TraceNumber(n),'int32'); %13-16 Trace number within the original field record. Highly recommended for all types of data.
    fwrite(fId,Head.EnergySourcePoint(n),'int32'); %17-20 Energy source point number — Used when more than one record occurs at the same effective surface location.  It is recommended that the new entry defined in Trace Header bytes 197-202 be used for shotpoint number.
    fwrite(fId,Head.cdp(n),'int32'); %21-24 Ensemble number (i.e. CDP, CMP, CRP, etc)
    fwrite(fId,Head.cdpTrace(n),'int32'); %25-28 Trace number within the ensemble — Each ensemble starts with trace number one.
    fwrite(fId,Head.TraceIdenitifactionCode(n),'int16'); %29-30 Trace identification code: -1=Other; 0=Unknown; 1=Seismic data; 2=Dead; 3=Dummy; 4=Time break; 5=Uphole; 6=Sweep; 7=Timing; 8=Waterbreak; 9=Near-field gun signature; 10=Far-field gun signature; 11=Seismic pressure sensor; 12=Multicomponent seismic sensor - Vertical component; 13=Multicomponent seismic sensor - Cross-line component; 14=Multicomponent seismic sensor - In-line component; 15=Rotated multicomponent seismic sensor - Vertical component; 16=Rotated multicomponent seismic sensor - Transverse component; 17=Rotated multicomponent seismic sensor - Radial component; 18=Vibrator reaction mass; 19=Vibrator baseplate; 20=Vibrator estimated ground force; 21=Vibrator reference; 22=Time-velocity pairs; 23 … N=optional use,  (maximum N = 32,767). Highly recommended for all types of data.
    fwrite(fId,Head.NSummedTraces(n),'int16'); %31-32 Number of vertically summed traces yielding this trace.  (1 is one trace, 2 is two summed traces, etc.)
    fwrite(fId,Head.NStackedTraces(n),'int16'); %33-34 Number of horizontally stacked traces yielding this trace.  (1 is one trace, 2 is two stacked traces, etc.)
    fwrite(fId,Head.DataUse(n),'int16'); %35-36 Data use: 1=Production; 2=Test.
    fwrite(fId,Head.offset(n),'int32'); %37-40 Distance from center of the source point to the center of the receiver group (negative if opposite to direction in which line is shot).
    ...
    fwrite(fId,Head.ReceiverGroupElevation(n),'int32'); %41-44 ////Receiver group elevation (all elevations above the Vertical datum are positive and below are negative).
    fwrite(fId,Head.SourceSurfaceElevation(n),'int32'); %45-48 ////Surface elevation at source.
    fwrite(fId,Head.SourceDepth(n),'int32'); %////49-52 Source depth below surface (a positive number).
    fwrite(fId,Head.ReceiverDatumElevation(n),'int32'); %53-56 ////Datum elevation at receiver group.
    fwrite(fId,Head.SourceDatumElevation(n),'int32'); %57-60 ////Datum elevation at source.
    fwrite(fId,Head.SourceWaterDepth(n),'int32'); %61-64 ////Water depth at source.
    fwrite(fId,Head.GroupWaterDepth(n),'int32'); %65-68 ////Water depth at group.
    fwrite(fId,Head.ElevationScalar(n),'int16'); %69-70 Scalar to be applied to all elevations and depths specified in Trace Header bytes 41-68 to give the real value.  Scalar = 1, +10, +100, +1000, or +10,000.  If positive, scalar is used as a multiplier; if negative, scalar is used as a divisor.
    fwrite(fId,Head.SourceGroupScalar(n),'int16'); %71-72 Scalar to be applied to all coordinates specified in Trace Header bytes 73-88 and to bytes Trace Header 181-188 to give the real value.  Scalar = 1, +10, +100, +1000, or +10,000.  If positive, scalar is used as a multiplier; if negative, scalar is used as divisor.
    ...
    fwrite(fId,Head.SourceX(n),'int32'); %//73-76 Source coordinate - X.
    fwrite(fId,Head.SourceY(n),'int32'); %//77-80 Source coordinate - Y.
    fwrite(fId,Head.GroupX(n),'int32'); %//81-84 Group coordinate - X.
    fwrite(fId,Head.GroupY(n),'int32'); %//85-88 Group coordinate - Y.
    fwrite(fId,Head.CoordinateUnits(n),'int16'); %89-90 Coordinate units: 1=Length (meters or feet); 2=Seconds of arc; 3=Decimal degrees; 4=Degrees, minutes, seconds (DMS). Note: To encode +-DDDMMSS bytes 89-90 equal= +-DDD*10^4 + MM*10^2 + SS with bytes 71-72 set to 1; To encode +-DDDMMSS.ss bytes 89-90 equal= +-DDD*10^6 + MM*10^4 + SS*10^2 with bytes 71-72 set to -100.
    fwrite(fId,Head.WeatheringVelocity(n),'int16'); %91-92 Weathering velocity. (ft/s or m/s as specified in Binary File Header bytes 3255-3256).
    fwrite(fId,Head.SubWeatheringVelocity(n),'int16'); %93-94 Subweathering velocity. (ft/s or m/s as specified in Binary File Header bytes 3255-3256)
    %////Time in milliseconds as scaled by the scaled by the scalar specified in Trace Header bytes 215-216.
    fwrite(fId,Head.SourceUpholeTime(n),'int16'); %95-96 ////Uphole time at source in milliseconds.
    fwrite(fId,Head.GroupUpholeTime(n),'int16'); %97-98 ////Uphole time at group in milliseconds.
    fwrite(fId,Head.SourceStaticCorrection(n),'int16'); %99-100 ////Source static correction in milliseconds.
    fwrite(fId,Head.GroupStaticCorrection(n),'int16'); %101-102 ////Group static correction in milliseconds.
    fwrite(fId,Head.TotalStaticApplied(n),'int16'); %103-104 ////Total static applied in milliseconds. (Zero if no static has been applied).
    fwrite(fId,Head.LagTimeA(n),'int16'); %105-106 ////Lag time A — Time in milliseconds between end of 240-byte trace identification header and time break.  The value is positive if time break occurs after the end of header; negative if time break occurs before the end of header.  Time break is defined as the initiation pulse that may be recorded on an auxiliary trace or as otherwise specified by the recording system.
    fwrite(fId,Head.LagTimeB(n),'int16'); %107-108 ////Lag Time B — Time in milliseconds between time break and the initiation time of the energy source.  May be positive or negative.
    fwrite(fId,Head.DelayRecordingTime(n),'int16'); %109-110 ////Delay recording time — Time in milliseconds between initiation time of energy source and the time when recording of data samples begins.  In SEG Y rev 0 this entry was intended for deep-water work if data recording does not start at zero time.  The entry can be negative to accommodate negative start times (i.e. data recorded before time zero, presumably as a result of static application to the data trace).  If a non-zero value (negative or positive) is recorded in this entry, a comment to that effect should appear in the Textual File Header.
    fwrite(fId,Head.MuteTimeStart(n),'int16'); %111-112 ////Mute time — Start time in milliseconds.
    fwrite(fId,Head.MuteTimeEnd(n),'int16'); %113-114 ////Mute time — End time in milliseconds.
    fwrite(fId,Head.ns(n),'int16'); %115-116 Number of samples in this trace. Highly recommended for all types of data.
    fwrite(fId,Head.dt(n),'int16'); %117-118 Sample interval in microseconds (µs) for this trace. The number of bytes in a trace record must be consistent with the number of samples written in the trace header.  This is important for all recording media; but it is particularly crucial for the correct processing of SEG Y data in disk files (see Appendix C). If the fixed length trace flag in bytes 3503-3504 of the Binary File Header is set, the sample interval and number of samples in every trace in the SEG Y file must be the same as the values recorded in the Binary File Header.  If the fixed length trace flag is not set, the sample interval and number of samples may vary from trace to trace. Highly recommended for all types of data.
    fwrite(fId,Head.GainType(n),'int16'); %119-120 Gain type of field instruments: 1=fixed; 2=binary; 3=floating point; 4 … N=optional use.
    fwrite(fId,Head.InstrumentGainConstant(n),'int16'); %121-122 Instrument gain constant (dB).
    fwrite(fId,Head.InstrumentInitialGain(n),'int16'); %123-124 Instrument early or initial gain (dB).
    fwrite(fId,Head.Correlated(n),'int16'); %125-126 Correlated: 1=no; 2=yes.
    fwrite(fId,Head.SweepFrequenceStart(n),'int16'); %127-128 Sweep frequency at start (Hz).
    fwrite(fId,Head.SweepFrequenceEnd(n),'int16'); %129-130 Sweep frequency at end (Hz).
    fwrite(fId,Head.SweepLength(n),'int16'); %131-132 Sweep length in milliseconds.
    fwrite(fId,Head.SweepType(n),'int16'); %S133-134 weep type: 1=linear; 2=parabolic; 3=exponential; 4=other.
    fwrite(fId,Head.SweepTraceTaperLengthStart(n),'int16'); %135-136 Sweep trace taper length at start in milliseconds.
    fwrite(fId,Head.SweepTraceTaperLengthEnd(n),'int16'); %137-138 Sweep trace taper length at end in milliseconds.
    fwrite(fId,Head.TaperType(n),'int16'); %139-140 Taper type: 1=linear; 2=cos^2; 3=other.
    fwrite(fId,Head.AliasFilterFrequency(n),'int16'); %141-142 Alias filter frequency (Hz), if used.
    fwrite(fId,Head.AliasFilterSlope(n),'int16'); %143-144 Alias filter slope (dB/octave).
    fwrite(fId,Head.NotchFilterFrequency(n),'int16'); %145-146 Notch filter frequency (Hz), if used.
    fwrite(fId,Head.NotchFilterSlope(n),'int16'); %147-148 Notch filter slope (dB/octave).
    fwrite(fId,Head.LowCutFrequency(n),'int16'); %149-150 Low-cut frequency (Hz), if used.
    fwrite(fId,Head.HighCutFrequency(n),'int16'); %151-152 High-cut frequency (Hz), if used.
    fwrite(fId,Head.LowCutSlope(n),'int16'); %153-154 Low-cut slope (dB/octave).
    fwrite(fId,Head.HighCutSlope(n),'int16'); %155-156 High-cut slope (dB/octave).
    fwrite(fId,Head.YearDataRecorded(n),'int16'); %157-158 Year data recorded — The 1975 standard is unclear as to whether this should be recorded as a 2-digit or a 4-digit year and both have been used.  For SEG Y revisions beyond rev 0, the year should be recorded as the complete 4-digit Gregorian calendar year (i.e. the year 2001 should be recorded as 2001 (7D1)).
    fwrite(fId,Head.DayOfYear(n),'int16'); %159-160 Day of year (Julian day for GMT and UTC time basis).
    fwrite(fId,Head.HourOfDay(n),'int16'); %161-162 Hour of day (24 hour clock).
    fwrite(fId,Head.MinuteOfHour(n),'int16'); %163-164 Minute of hour.
    fwrite(fId,Head.SecondOfMinute(n),'int16'); %165-166 Second of minute.
    fwrite(fId,Head.TimeBaseCode(n),'int16'); %167-168 Time basis code: 1=Local; 2=GMT (Greenwich Mean Time); 3=Other, should be explained in a user defined stanza in the Extended Textual File Header; 4=UTC (Coordinated Universal Time).
    fwrite(fId,Head.TraceWeightningFactor(n),'int16'); %169-170 Trace weighting factor — Defined as 2^-N volts for the least significant bit.  (N = 0, 1, ..., 32767)
    fwrite(fId,Head.GeophoneGroupNumberRoll(n),'int16'); %171-172 Geophone group number of roll switch position one.
    fwrite(fId,Head.GeophoneGroupNumberFirstTraceOrigField(n),'int16'); %173-174 Geophone group number of trace number one within original field record.
    fwrite(fId,Head.GeophoneGroupNumberLastTraceOrigField(n),'int16'); %175-176 Geophone group number of last trace within original field record.
    fwrite(fId,Head.GapSize(n),'int16'); %177-178 Gap size (total number of groups dropped).
    fwrite(fId,Head.OverTravel(n),'int16'); %179-180 Over travel associated with taper at beginning or end of line: 1=down (or behind); 2=up (or ahead).
    fwrite(fId,Head.cdpX(n),'int32'); %181-184 X coordinate of ensemble (CDP) position of this trace (scalar in Trace Header bytes 71-72 applies). The coordinate reference system should be identified through an extended header Location Data stanza (see section D-1).
    fwrite(fId,Head.cdpY(n),'int32'); %185-188 Y coordinate of ensemble (CDP) position of this trace (scalar in bytes Trace Header 71-72 applies). The coordinate reference system should be identified through an extended header Location Data stanza (see section D-1).
    fwrite(fId,Head.Inline3D(n),'int32'); %189-192 For 3-D poststack data, this field should be used for the in-line number. If one in-line per SEG Y file is being recorded, this value should be the same for all traces in the file and the same value will be recorded in bytes 3205-3208 of the Binary File Header.
    fwrite(fId,Head.Crossline3D(n),'int32'); %193-196 For 3-D poststack data, this field should be used for the cross-line number. This will typically be the same value as the ensemble (CDP) number in Trace Header bytes 21-24, but this does not have to be the case.
    fwrite(fId,Head.ShotPoint(n),'int32'); %197-200 Shotpoint number — This is probably only applicable to 2-D poststack data. Note that it is assumed that the shotpoint number refers to the source location nearest to the ensemble (CDP) location for a particular trace.  If this is not the case, there should be a comment in the Textual File Header explaining what the shotpoint number actually refers to.
    fwrite(fId,Head.ShotPointScalar(n),'int16'); %201-202 Scalar to be applied to the shotpoint number in Trace Header bytes 197-200 to give the real value.  If positive, scalar is used as a multiplier; if negative as a divisor; if zero the shotpoint number is not scaled (i.e. it is an integer.  A typical value will be -10, allowing shotpoint numbers with one decimal digit to the right of the decimal point).
    fwrite(fId,Head.TraceValueMeasurementUnit(n),'int16'); %203-204 Trace value measurement unit: -1=Other (should be described in Data Sample Measurement Units Stanza); 0=Unknown; 1=Pascal(Pa); 2=Volts(v); 3=Millivolts(mV); 4=Amperes(A); 5=Meters(m); 6=Meters per second(m/s); 7=Meters per second squared(m/s^2); 8=Newton(N); 9=Watt(W).
    fwrite(fId,Head.TransductionConstantMantissa(n),'int32'); %205-208 single for KP in SW//Transduction Constant — The multiplicative constant used to convert the Data Trace samples to the Transduction Units (specified in Trace Header bytes 211-212).  The constant is encoded as a four-byte, two's complement integer (bytes 205-208) which is the mantissa and a two-byte, two's complement integer (bytes 209-210) which is the power of ten exponent (i.e. Bytes 205-208 * 10**Bytes 209-210).
    fwrite(fId,Head.TransductionConstantPower(n),'int16'); %209-210//
    fwrite(fId,Head.TransductionUnit(n),'int16'); %211-212 Transduction Units — The unit of measurement of the Data Trace samples after they have been multiplied by the Transduction Constant specified in Trace Header bytes 205-210. -1=Other (should be described in Data Sample Measurement Unit stanza, page 36); 0=Unknown; 1=Pascal(Pa); 2=Volts(v); 3=Millivolts(mV); 4=Amperes(A); 5=Meters(m); 6=Meters per second(m/s); 7=Meters per second squared(m/s^2); 8=Newton(N); 9=Watt(W).
    fwrite(fId,Head.TraceIdentifier(n),'int16'); %213-214 Device/Trace Identifier — The unit number or id number of the device associated with the Data Trace (i.e. 4368 for vibrator serial number 4368 or 20316 for gun 16 on string 3 on vessel 2).  This field allows traces to be associated across trace ensembles independently of the trace number (Trace Header bytes 25-28).
    fwrite(fId,Head.ScalarTraceHeader(n),'int16'); %215-216 Scalar to be applied to times specified in Trace Header bytes 95-114 to give the true time value in milliseconds.  Scalar = 1, +10, +100, +1000, or +10,000. If positive, scalar is used as a multiplier; if negative, scalar is used as divisor. A value of zero is assumed to be a scalar value of 1.
    fwrite(fId,Head.SourceType(n),'int16'); %217-218 Source Type/Orientation — Defines the type and the orientation of the energy source.  The terms vertical, cross-line and in-line refer to the three axes of an orthogonal coordinate system.  The absolute azimuthal orientation of the coordinate system axes can be defined in the Bin Grid Definition Stanza (page 27). -1 to -n = Other (should be described in Source Type/Orientation stanza, page 38). 0=Unknown; 1=Vibratory - Vertical orientation; 2=Vibratory - Cross-line orientation; 3=Vibratory - In-line orientation; 4=Impulsive - Vertical orientation; 5=Impulsive - Cross-line orientation; 6=Impulsive - In-line orientation; 7=Distributed Impulsive - Vertical orientation; 8=Distributed Impulsive - Cross-line orientation; 9=Distributed Impulsive - In-line orientation.
    fwrite(fId,Head.SourceEnergyDirectionMantissa(n),'int32'); %219-222 ////Source Energy Direction with respect to the source orientation  — The positive orientation direction is defined in Bytes 217-218 of the Trace Header. The energy direction is encoded in tenths of degrees (i.e. 347.8 is encoded as 3478).
    fwrite(fId,Head.SourceEnergyDirectionExponent(n),'int16'); %223-224 ////
    fwrite(fId,Head.SourceMeasurementMantissa(n),'int32'); %225-228 //Source Measurement — Describes the source effort used to generate the trace. The measurement can be simple, qualitative measurements such as the total weight of explosive used or the peak air gun pressure or the number of vibrators times the sweep duration.  Although these simple measurements are acceptable, it is preferable to use true measurement units of energy or work. The constant is encoded as a four-byte, two's complement integer (bytes 225-228) which is the mantissa and a two-byte, two's complement integer (bytes 209-230) which is the power of ten exponent (i.e. Bytes 225-228 * 10**Bytes 229-230).
    fwrite(fId,Head.SourceMeasurementExponent(n),'int16'); %229-230 //
    fwrite(fId,Head.SourceMeasurementUnit(n),'int16'); %231-232 Source Measurement Unit — The unit used for the Source Measurement, Trace header bytes 225-230. -1 = Other (should be described in Source Measurement Unit stanza, page 39); 0=Unknown; 1=Joule(J); 2=Kilowatt(kW); 3=Pascal(Pa); 4=Bar(Bar); 4=Bar-meter(Bar-m); 5=Newton(N); 6=Kilograms(kg).
    fwrite(fId,Head.UnassignedInt1(n),'int32'); %233-236 Unassigned — For optional information.
    fwrite(fId,Head.UnassignedInt2(n),'int32'); %237-240 Unassigned — For optional information.
