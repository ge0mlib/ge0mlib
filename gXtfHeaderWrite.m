function gXtfHeaderWrite(XtfHead,fNameNew)
%Write XtfHead structure (include XTFFILEHEADER, CHANINFO and Short Header) to New *.xtf file.
%function gXtfHeaderWrite(XtfHead,fNameNew), where
%XtfHead- structure with parent xtf-file description; 
%fNameNew- string, the target file for writing.
%gXtfHeaderWrite used file XtfHead.fName as data source. The file XtfHead.fName must be presented; names XtfHead.fName and fNameNew must be different.
%You can use XtfHead.ROnFlag to except any record from new ftx-file (see gXtfHeaderMessOff). Warning: for new file CHANINFO Structure not controlled; if necessary, remove Chanel Records in handle.
%Example: gXtfHeaderWrite(XtfHead,'c:\temp\1new.xtf');

[fId, mes]=fopen(XtfHead.fName,'r+');if ~isempty(mes), error(['gFXtfWriteHeader: ' mes]);end;
if strcmp(fNameNew,XtfHead.fName), error('gFXtfWriteHeader: XtfHead.fName and fNameNew must be different.');end;%if old and new name of file the same -- rewriting all XtfHeader Structures from XtfHead structure to parent file
