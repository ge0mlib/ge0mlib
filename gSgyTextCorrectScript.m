...
%The 16th mandatory records marked in comments as: !!!!MANDATORY!!!!
...
%Start script with command >>> {'d:\3200SX\'};gSgyTextCorrectScript; <<< or the same.
...
%====C01====
n=01;StHead(n).Pos=012;StHead(n).Num=22;StHead(n).Rec='COMPANY1'; %!!!!MANDATORY!!!! \\ CLIENT \\ Заказчик
...
%====C02====
n=04;StHead(n).Pos=090;StHead(n).Num=10;StHead(n).Rec='NW'; %!!!!MANDATORY!!!! \\ LINE W/SURVEY PREFIX \\ Имя профиля или съемки
n=05;StHead(n).Pos=106;StHead(n).Num=25;StHead(n).Rec='Some Sea'; %!!!!MANDATORY!!!! \\ AREA \\ Площадь
n=06;StHead(n).Pos=136;StHead(n).Num=25;StHead(n).Rec='WGS84;6378137;298.2572236'; %!!!!MANDATORY!!!! \\ MAP ID/INT.DATUM \\ Проекция, эллипсоид
%====C03====
...
%====C05====
n=14;StHead(n).Pos=344;StHead(n).Num=06;StHead(n).Rec='1'; %!!!!MANDATORY!!!! \\ DATA TRACES/RECORD \\ Количество трасс на запись
n=15;StHead(n).Pos=375;StHead(n).Num=07;StHead(n).Rec='0'; %!!!!MANDATORY!!!! \\ AUXILIARY TRACES/RECORD \\ Вспомогательные трассы на запись
n=16;StHead(n).Pos=392;StHead(n).Num=09;StHead(n).Rec='1'; %CDP FOLD \\ Кратность
%====C06====
n=17;StHead(n).Pos=421;StHead(n).Num=06;StHead(n).Rec='46'; %!!!!MANDATORY!!!! \\ SAMPLE INTERVAL (mks) \\ Шаг дискретизации в мкс
n=18;StHead(n).Pos=443;StHead(n).Num=05;StHead(n).Rec='#'; %!!!!MANDATORY!!!! \\ SAMPLES/TRACE \\ Количество отсчетов на трассу
...
%====C07====
n=21;StHead(n).Pos=502;StHead(n).Num=06;StHead(n).Rec='SEGY1'; %!!!!MANDATORY!!!! \\ RECORDING FORMAT \\ Формат записи
n=22;StHead(n).Pos=526;StHead(n).Num=06;StHead(n).Rec='NTFS'; %!!!!MANDATORY!!!! \\ FORMAT THIS REEL \\ Формат магнитного носителя
...
%====C21====
n=73;StHead(n).Pos=1627;StHead(n).Num=19;StHead(n).Rec='not applicable'; %!!!!MANDATORY!!!! \\ CONTRACTOR AND SOFTWARE VERSION USED TO PERFORM THE DEMULTIPLEXING \\ Наименование организации-обработчика и программного обеспечения, применявшегося для демультиплексации
%====C27====
n=74;StHead(n).Pos=2097;StHead(n).Num=64;StHead(n).Rec='500000.00E; 0.00N'; %!!!!MANDATORY!!!! \\ GRID ORIG 0 \\ Начало координатной сетки
%====C31====
n=75;StHead(n).Pos=2440;StHead(n).Num=41;StHead(n).Rec='not applicable'; %!!!!MANDATORY!!!! \\ I-LINE X-LINE BIN SIZE AND AZIMUTH \\ Размеры площадки Bin, инлайна и крослайна и азимут.
%====C32====
n=76;StHead(n).Pos=2510;StHead(n).Num=51;StHead(n).Rec='not applicable'; %!!!!MANDATORY!!!! \\ I-LINE X-LINE INCREMENTS \\ Инкремент инлайна и крослайна.
%====C37====
...
%====C39====
n=79;StHead(n).Pos=3045;StHead(n).Num=76;StHead(n).Rec='SEG Y REV1'; %!!!!MANDATORY!!!! \\ SEG-Y REV \\ Ревизия формата
%====C40====
n=80;StHead(n).Pos=3125;StHead(n).Num=76;StHead(n).Rec='END EBCDIC'; %!!!!MANDATORY!!!! \\ END EBCDIC \\ Конец заголовка EBCDIC
%====END====
