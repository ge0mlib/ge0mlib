...
SgyHead.dt=fread(fId,1,'int16'); %1000000/ADC_Freq // Sample interval in microseconds (µs).  Mandatory for all data types.
SgyHead.dtOrig=fread(fId,1,'int16'); %1000000/ADC_Freq // Sample interval in microseconds  (µs) of original field recording.
...
SgyHead.VerticalSumCode=fread(fId,1,'int16'); %Vertical sum code: 1 = no sum, 2 = two sum, Е, N = M-1 sum  (M = 2 to 32,767).
...
SgyHead.VibratoryPolarityCode=fread(fId,1,'int16'); %Vibratory polarity code: Seismic signal lags pilot signal by: 1=337.5 to 22.5; 2=22.5 to 67.5; 3=67.5 to 112.5; 4=112.5 to 157.5; 5=157.5 to 202.5; 6=202.5 to 247.5; 7=247.5 to 292.5;8=292.5 to 337.5.
 ...
 SgyHead.nGLS_Sign=fread(fId,1,'int32'); %!!!added = 44534c47h или 1146309703 for files were MAK-created
...
for n=1:NTrace,
    ...
    Head.MessageNum(n)=n;
    Head.TraceSequenceLine(n)=fread(fId,1,'int32'); %Trace sequence number within line Ч Numbers continue to increase if the same line continues across multiple SEG Y files. Highly recommended for all types of data.
    Head.TraceSequenceFile(n)=fread(fId,1,'int32'); %Trace sequence number within SEG Y file Ч Each file starts with trace sequence one.
    ...
    Head.EnergySourcePoint(n)=fread(fId,1,'int32'); %Energy source point number Ч Used when more than one record occurs at the same effective surface location.  It is recommended that the new entry defined in Trace Header bytes 197-202 be used for shotpoint number.
    Head.cdp(n)=fread(fId,1,'int32'); %Ensemble number (i.e. CDP, CMP, CRP, etc)
    Head.cdpTrace(n)=fread(fId,1,'int32'); %Trace number within the ensemble Ч Each ensemble starts with trace number one.
    Head.TraceIdenitifactionCode(n)=fread(fId,1,'int16'); %=1 //Trace identification code: -1=Other; 0=Unknown; 1=Seismic data; 2=Dead; 3=Dummy; 4=Time break; 5=Uphole; 6=Sweep; 7=Timing; 8=Waterbreak; 9=Near-field gun signature; 10=Far-field gun signature; 11=Seismic pressure sensor; 12=Multicomponent seismic sensor - Vertical component; 13=Multicomponent seismic sensor - Cross-line component; 14=Multicomponent seismic sensor - In-line component; 15=Rotated multicomponent seismic sensor - Vertical component; 16=Rotated multicomponent seismic sensor - Transverse component; 17=Rotated multicomponent seismic sensor - Radial component; 18=Vibrator reaction mass; 19=Vibrator baseplate; 20=Vibrator estimated ground force; 21=Vibrator reference; 22=Time-velocity pairs; 23 Е N=optional use,  (maximum N = 32,767). Highly recommended for all types of data.
    ...
    Head.GainType(n)=fread(fId,1,'int16'); %=1 // Gain type of field instruments: 1=fixed; 2=binary; 3=floating point; 4 Е N=optional use.
    ...
    Head.YearDataRecorded(n)=fread(fId,1,'int16'); %Year data recorded Ч The 1975 standard is unclear as to whether this should be recorded as a 2-digit or a 4-digit year and both have been used.  For SEG Y revisions beyond rev 0, the year should be recorded as the complete 4-digit Gregorian calendar year (i.e. the year 2001 should be recorded as 2001 (7D1)).
    ...
    Head.TraceWeightningFactor(n)=fread(fId,1,'int16'); %Trace weighting factor Ч Defined as 2^-N volts for the least significant bit.  (N = 0, 1, ..., 32767)
    Head.nChannelCode(n)=fread(fId,1,'int16'); %!!!changed Type of channel -- 1=subbottom, 2=port, 3=stbd
    Head.nShipCourse(n)=fread(fId,1,'uint16'); %!!!changed  урс судна в градусах * 100
    Head.nShipSpeed(n)=fread(fId,1,'uint16'); %!!!changed —корость судна в м/с * 100
    ...
    Head.nlMagZ(n)=fread(fId,1,'int32'); %!!!changed еЌ
    ...
    Head.nlCableout(n)=fread(fId,1,'uint16'); %!!!changed ƒлина кабель-тросса с точностью до 50см  chg 24.04.2005 // 09.02.2007 fix: с точностью до 25 см
    Head.nlSoundVelocity(n)=fread(fId,1,'int32'); %!!!changed —корость звука в м/с * 100
    Head.nlPressure_dBars(n)=fread(fId,1,'int32'); %!!!changed ƒавление в дЅарах * 1000
    Head.nlTemperature(n)=fread(fId,1,'int32'); %!!!changed “емпература в градусах цельси€ * 100
    Head.nlLayback(n)=fread(fId,1,'int32'); %!!!changed –ассто€ние по поверхности от корабл€ до рыбы в см
